...
- **Evidence and intent** (`evidence.ts`, `intent.ts`, `reality.ts` in shared) exist because every rule input is hand-typed. `validate(graph, { observations, intent, now })` resolves observation sets (matched to nodes by `ArchNode.ref`, falling back to id; freshest source wins per field; sets older than 24h are stale and not applied), runs the 13 rules against the *effective* graph (declared values with observed ones laid over them), and marks findings that only appear because of evidence as `observed`. Observed traffic makes nodes carrying at least 25% of the busiest node's rps count as critical, and demotes findings whose cited elements all show 0 rps, unless a cited node is *declared* critical (failover paths carry no traffic until they matter). Five extra checks (`REALITY_CHECKS`) compare drawn, running, and approved values three ways: `observed-drift`, `unapproved-change`, `timeout-below-latency`, `undiagrammed-dependency`, `stale-evidence`. They can be muted by id like rules. With no observations and no baseline, output is identical to before.
- Observations enter via `POST /api/rooms/:roomId/observations` (Zod-bounded, one set per `source`, replaced wholesale), which writes into the room's `observations` Y.Map through `RoomManager.mutate()` and flushes before responding, so every open canvas re-validates live. The approved baseline lives in the `intent` Y.Map (one nested map per element, one key per field, so concurrent approvals merge) and is included in the `UndoManager` scope, so "Match running system" (edit + approve) undoes as one step. Only runtime-meaningful fields are tracked (`NODE_INTENT_FIELDS`/`EDGE_INTENT_FIELDS`); moving or renaming a box never counts as an unapproved change.
